...
from library.qt_lib.thread import ChromeThread, MyThread, CheckChromeStatus
...
class HomeInterface(QWidget, Ui_Form):
    ...
    def test(self):
        ...
        data = model.data(model.index(0, db_data.DB_ENV_NUM_COL))
        log.debug(data)
        # log.debug(pprint.pformat(self.started_chrome_info_dict))
        # thread = CheckChromeStatus(self.started_chrome_info_dict)
        # thread.status_change.connect(self.change_chrome_status)
        # thread.start()
        # self.thread_list.append(thread)

    def change_chrome_status(self, res):
        status, index = res
        if status:
            log.success('状态改变：已启动')
            self.started_chrome_info_dict[index].chrome_status = status
            handle_env_config_db.set_chrome_status(self, self.started_chrome_info_dict[index].env_num, '已启动')
            self.tableView_env_config.model().select()
        else:
            log.success('状态改变：关闭')
            handle_env_config_db.set_chrome_status(self, self.started_chrome_info_dict[index].env_num, '已关闭')
            self.tableView_env_config.model().select()
            self.close_chrome(int(index))
    ...
    def run_script(self):
        ...
        # 浏览器分组
        env_num_group_list = [list(range(i, min(i + thread_count, _to + 1))) for i in
                              range(_from, _to + 1, thread_count)]
        ...
        self.run_group_chrome(env_num_group_list, all_script_name)
    ...
    def run_group_chrome(self, env_num_group_list, all_script_name):
        ...
        self.run_env_list = run_env_list
        ...
        for index in run_env_list:
            ...
            self.start_new_chrome(chrome_info, env_num_group_list, all_script_name)
    ...
    def handle_start_action(self, selected_indexes):
        try:
            ...
            for index in selected_row_list:
                ...
                chrome_info.env_num = int(model.data(model.index(index, db_data.DB_ENV_NUM_COL)))
                chrome_info.proxy_info = model.data(model.index(index, db_data.DB_PROXY_COL))
                self.start_new_chrome(chrome_info)
        except Exception as e:
            log.error(e)

    def start_new_chrome(self, chrome_info: ChromeInfo, env_num_group_list: list = None, all_script_name: list = None):
        def handle_start_chrome_result(return_chrome_info):
            ...
            # 判断是脚本执行还是开启浏览器
            if env_num_group_list is not None and len(env_num_group_list) > 0:
                self.show_info.create_success_info_bar(f'浏览器：{return_chrome_info.env_num}脚本执行完成')
                self.run_env_list.remove(env_num)
                if not self.run_env_list:
                    self.run_group_chrome(env_num_group_list, all_script_name)
            elif env_num_group_list is not None and len(env_num_group_list) == 0:
                self.show_info.create_success_info_bar(f'浏览器：{return_chrome_info.env_num}脚本执行完成',
                                                       '脚本全部执行完成')
            else:
        ...
        log.info('启动chrome...')
        thread = ChromeThread(chrome_info, self.started_chrome_info_dict, all_script_name)
        thread.result_ready.connect(handle_start_chrome_result)
        thread.start()
        self.thread_list.append(thread)
